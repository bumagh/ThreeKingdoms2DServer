...
char* err = NULL;
char* dbpath =
"/root/work/gitrep/lwan/src/samples/ThreeKingdoms2DServer/data/user.db";
...
static int
selectjson_callback (void* data, int argc, char** argv, char** azColName)
{
    int i;
    ...
    if(strlen (jsonbuf) == 0)
    {
        ...
        yyjson_mut_val* ele = yyjson_mut_obj_with_str (
            doc, (const char**)azColName, (const char**)argv, argc);
    ...
    else
    {
        ...
        if(doc)
        {
            ...
            if(yyjson_is_arr (mutroot))
            {
                ...
                yyjson_mut_val* ele =
                    yyjson_mut_obj_with_str (mutdoc, (const char**)azColName,
...
static int
write_callback (void* NotUsed, int argc, char** argv, char** azColName)
{
    int i;
    for(i = 0; i < argc; i++)
    {
...
int selectLogin (char* account, char* pwd)
{
    ...
    /* Execute SQL statement */
    selectRet =
        sqlite3_exec (db, buf, select_login_callback, (void*)&ret, &zErrMsg);
    if(selectRet != SQLITE_OK)
    {
        fprintf (stderr, "SQL error: %s\n", zErrMsg);
        sqlite3_free (zErrMsg);
...
int select_user_info_all (
    char* key, int key_len, char* val, int val_len, char* bufstr)
{
    sqlite3* db;
    char* zErrMsg = 0;
    int rc;
    ...
    /* Execute SQL statement */
    selectRet =
        sqlite3_exec (db, buf, selectjson_callback, (void*)&jsonbuf, &zErrMsg);
    if(selectRet != SQLITE_OK)
    {
        fprintf (stderr, "SQL error: %s\n", zErrMsg);
        sqlite3_free (zErrMsg);
...
int updateKvById (
    int user_id, char* key, int key_len, void* val, int val_len, int val_type)
{
    sqlite3* db;
    char* zErrMsg = 0;
    int rc;
